...
We are particularly interested to identify those _known_ or _unknown_  populations that are out-of-distribution and potentially are extreme cases for a deployed artificial intelligent model. 
...
### Results
Figures below show the mean of Mahalanobis distances as outlier scores and the relevant distributions for various populations including MNIST (training), MNIST(validation), MNIST (known outlier), Fashion MNIST (unknown outlier), and CIFAR10 (unknown outlier).

We obtained that the Mahalanobis outlier detector predicts larger mean scores for outliers than those inliers of which used for training and validation.
This clearly indicates that Mahalanobis detector enables us to detect outliers either there exist within the training dataset or completely belong to external datasets.
...
<figure style="width: 600px" class="align-center">
  ...
  <figcaption>Mean outlier score (i.e. Mahalanobis distance) for various populations of MNIST (training), MNIST(validation), MNIST (known outlier), Fashion MNIST (unknown outlier), and CIFAR10 (unknown outlier).</figcaption>
</figure> 


Further insights can be obtained by considering an overlaid box and violin plots of evaluated distances which shows how outlier scores are distributed for different populations.
It shows that there are still some outliers with small scores in compare with inliers.
...
The histogram representation of the scores for the mentioned subsets demonstrates that there are overlap regions that the Mahalanobis detector unable to identify all outliers. Nevertheless, detecting the shifts in data distribution due to outliers is pretty much noticeable.
